...
public class UserController {
	...
	public String addToShoppingCart(Model model, @RequestParam Integer accountId, @RequestParam Integer deviceId) {
	...
	public String shoppingCart(Model model) {
		...
		logger.info(totalCostOfDevices);
		model.addAttribute("userDevices", userDevices);
		model.addAttribute("user", user);
		model.addAttribute("totalCostOfDevices", totalCostOfDevices);
		return "home/shopping_cart";
	}

	@PostMapping(path = { "/buy_shopping_cart" })
	public String addToShoppingCart(Model model, @RequestParam Integer userId, @RequestParam Integer totalCostOfDevices,
			@RequestParam(required = false) Integer totalCostOfTrainingPlans, RedirectAttributes redirectAttributes) {
		logger.info("totalCostOfTrainingPlans " + totalCostOfTrainingPlans);
		Account user = accountService.findById(userId).get();
		Set<UserDevice> userDevices = userDeviceService.findAllByBoughtAndUserAccountId(false, user.getAccountId());
		if (user.getTransaction().getAvailableBalance() >= totalCostOfDevices) {
			for (UserDevice userDevice : userDevices) {
				Integer devicePrice = userDevice.getDevice().getPrice();
				user.getTransaction().setAvailableBalance(user.getTransaction().getAvailableBalance() - devicePrice);
				user.getTransaction().setPayments(-totalCostOfDevices);
